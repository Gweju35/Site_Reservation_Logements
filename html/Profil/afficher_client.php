...
try {
    ...
    NATURAL JOIN alhaiz_breizh._client
    WHERE compte_id = :client_id";
    $stmt = $pdo->prepare($sql);
    $stmt->bindParam(':client_id', $_POST['id_compte']); 
    $stmt->execute();

    $user = $stmt->fetch(PDO::FETCH_ASSOC);

    // Récuperer donné de la table avis
    $sql = "SELECT COUNT(*) FROM alhaiz_breizh._avis WHERE avis_id_compte_client = :client_id";
    $stmt = $pdo->prepare($sql);
    $stmt->bindParam(':client_id',  $_POST['id_compte']);
    $stmt->execute();
